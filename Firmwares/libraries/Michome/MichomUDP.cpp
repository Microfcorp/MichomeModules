...
void MichomeUDP::init(ESP8266WebServer& server2){
    ...
    server1.on("/udptrigger", [&](){
        if(server1.arg("type") == "show" || !server1.hasArg("type")){
            ...
            server1.sendContent(F("</table><br /><a href='udptrigger?type=add'>Добавить</a>"));
        }
...
void MichomeUDP::Load(void){
    ...
    for(int i = 0; i < countQ; i++){
        ...
        UDPTriggers qq = {(Split(str, '~', 0)), ((ActionsType)Split(str, '~', 1).toInt()), (Split(str, '~', 2)), (Split(str, '~', 3).toInt() == 1)};
        Ut.add(qq);
    }
}
